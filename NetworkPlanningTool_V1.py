...
class PCIGUIApp:
    ...
    def create_param_update_tab(self):
        ...
        # NR Search parameters - simplified: only query value input
        search_param_frame = ttk.Frame(nr_search_frame)  # New frame for search parameters to keep them compact
        search_param_frame.grid(row=0, column=0, columnspan=5, sticky=tk.W, padx=5, pady=5)
        ttk.Label(search_param_frame, text="查询值:").pack(side=tk.LEFT)
        self.nr_search_entry = PlaceholderEntry(search_param_frame, placeholder="输入gNodeB标识或小区名称", color="black", textvariable=self.nr_search_value, width=50)
        self.nr_search_entry.pack(side=tk.LEFT, padx=5)
        self.nr_search_btn = ttk.Button(search_param_frame, text="查询", command=self.search_nr_cells)
        self.nr_search_btn.pack(side=tk.LEFT, padx=5)

        # NR Results table
        nr_result_frame = ttk.LabelFrame(nr_search_frame, text="NR查询结果")
        nr_result_frame.grid(row=1, column=0, columnspan=5, sticky=tk.NSEW, padx=1, pady=1)
        nr_result_frame.columnconfigure(0, weight=1)
        nr_result_frame.rowconfigure(0, weight=1)

        # Create Treeview for NR results
        nr_columns = ("gNodeB标识", "小区经度", "小区纬度", "小区名称", "小区标识", "天线挂高", "天线方向角", "小区覆盖类型", "填写SSB频点", "物理小区识别码", "移动网络码", "第一分组", "子网ID")
        self.nr_result_tree = ttk.Treeview(nr_result_frame, columns=nr_columns, show="headings", height=18)

        # Set up column headings and widths
        for col in nr_columns:
            self.nr_result_tree.heading(col, text=col)
            self.nr_result_tree.column(col, width=100, anchor=tk.W)

        # Add scrollbars
        nr_v_scrollbar = ttk.Scrollbar(nr_result_frame, orient=tk.VERTICAL, command=self.nr_result_tree.yview)
        nr_h_scrollbar = ttk.Scrollbar(nr_result_frame, orient=tk.HORIZONTAL, command=self.nr_result_tree.xview)
        self.nr_result_tree.configure(yscrollcommand=nr_v_scrollbar.set, xscrollcommand=nr_h_scrollbar.set)

        # Pack the widgets
        self.nr_result_tree.grid(row=0, column=0, sticky='nsew')
        nr_v_scrollbar.grid(row=0, column=1, sticky='ns')
        nr_h_scrollbar.grid(row=1, column=0, sticky='ew')

        # Configure grid weights for resizing
        nr_result_frame.grid_rowconfigure(0, weight=1)
        nr_result_frame.grid_columnconfigure(0, weight=1)

        # Add right-click menu for copying cell values
        self.nr_copy_menu = tk.Menu(self.nr_result_tree, tearoff=0)
        self.nr_copy_menu.add_command(label="复制", command=self.copy_nr_cell_value)
        
        def nr_show_copy_menu(event):
            # Select the item under the cursor
            item = self.nr_result_tree.identify_row(event.y)
            if item:
                self.nr_result_tree.selection_set(item)
                # Get the column that was clicked
                column = self.nr_result_tree.identify_column(event.x)
                # Store the column index (remove the '#' prefix and convert to int)
                # Column index starts from 1 in Treeview (#1, #2, #3...)
                col_index = int(column.replace('#', '')) - 1
                self.nr_clicked_column = col_index
                self.nr_copy_menu.post(event.x_root, event.y_root)
        
        self.nr_result_tree.bind("<Button-3>", nr_show_copy_menu)  # Right-click on Windows
        self.nr_result_tree.bind("<Button-2>", nr_show_copy_menu)  # Right-click on macOS

        # LTE Cell Search tab
        lte_search_frame = ttk.Frame(self.search_notebook)
        self.search_notebook.add(lte_search_frame, text="LTE小区查询")
        # Configure LTE search frame to be responsive
        lte_search_frame.columnconfigure(0, weight=1)
        lte_search_frame.rowconfigure(1, weight=1)  # Row 1 is the result frame

        # LTE Search parameters - simplified: only query value input
        lte_search_param_frame = ttk.Frame(lte_search_frame)  # New frame for search parameters to keep them compact
        lte_search_param_frame.grid(row=0, column=0, columnspan=5, sticky=tk.W, padx=5, pady=5)
        ttk.Label(lte_search_param_frame, text="查询值:").pack(side=tk.LEFT)
        self.lte_search_entry = PlaceholderEntry(lte_search_param_frame, placeholder="输入eNodeB标识或小区名称", color="black", textvariable=self.lte_search_value, width=50)
        self.lte_search_entry.pack(side=tk.LEFT, padx=5)
        self.lte_search_btn = ttk.Button(lte_search_param_frame, text="查询", command=self.search_lte_cells)
        self.lte_search_btn.pack(side=tk.LEFT, padx=5)

        # LTE Results table
        lte_result_frame = ttk.LabelFrame(lte_search_frame, text="LTE查询结果")
        lte_result_frame.grid(row=1, column=0, columnspan=5, sticky=tk.NSEW, padx=1, pady=1)
        lte_result_frame.columnconfigure(0, weight=1)
        lte_result_frame.rowconfigure(0, weight=1)

        # Create Treeview for LTE results
        lte_columns = ("eNodeB标识", "小区经度", "小区纬度", "小区名称", "小区标识", "天线挂高", "天线方向角", "小区覆盖类型", "下行链路的中心载频", "物理小区识别码", "跟踪区码", "第一分组", "子网ID")
        self.lte_result_tree = ttk.Treeview(lte_result_frame, columns=lte_columns, show="headings", height=18)

        # Set up column headings and widths
        for col in lte_columns:
            self.lte_result_tree.heading(col, text=col)
            self.lte_result_tree.column(col, width=100, anchor=tk.W)

        # Add scrollbars
        lte_v_scrollbar = ttk.Scrollbar(lte_result_frame, orient=tk.VERTICAL, command=self.lte_result_tree.yview)
        lte_h_scrollbar = ttk.Scrollbar(lte_result_frame, orient=tk.HORIZONTAL, command=self.lte_result_tree.xview)
        self.lte_result_tree.configure(yscrollcommand=lte_v_scrollbar.set, xscrollcommand=lte_h_scrollbar.set)

        # Pack the widgets
        self.lte_result_tree.grid(row=0, column=0, sticky='nsew')
        lte_v_scrollbar.grid(row=0, column=1, sticky='ns')
        lte_h_scrollbar.grid(row=1, column=0, sticky='ew')

        # Configure grid weights for resizing
        lte_result_frame.grid_rowconfigure(0, weight=1)
        lte_result_frame.grid_columnconfigure(0, weight=1)

        # Add right-click menu for copying cell values
        self.lte_copy_menu = tk.Menu(self.lte_result_tree, tearoff=0)
        self.lte_copy_menu.add_command(label="复制", command=self.copy_lte_cell_value)
        
        def lte_show_copy_menu(event):
            # Select the item under the cursor
            item = self.lte_result_tree.identify_row(event.y)
            if item:
                self.lte_result_tree.selection_set(item)
                # Get the column that was clicked
                column = self.lte_result_tree.identify_column(event.x)
                # Store the column index (remove the '#' prefix and convert to int)
                # Column index starts from 1 in Treeview (#1, #2, #3...)
                col_index = int(column.replace('#', '')) - 1
                self.lte_clicked_column = col_index
                self.lte_copy_menu.post(event.x_root, event.y_root)
        
        self.lte_result_tree.bind("<Button-3>", lte_show_copy_menu)  # Right-click on Windows
        self.lte_result_tree.bind("<Button-2>", lte_show_copy_menu)  # Right-click on macOS
    ...
    def search_nr_cells(self):
        """Search for NR cells based on selected field and value"""
        try:
            # Clear previous results
            for item in self.nr_result_tree.get_children():
                self.nr_result_tree.delete(item)

            # Get the search parameters
            search_value = self.nr_search_value.get()

            if not search_value:
                messagebox.showwarning("NR小区查询", "请输入查询值！")
                return

            # Get the latest parameter file
            param_file = self.param_update_file.get()
            if not param_file:
                # Try to automatically find the latest parameter file
                import glob
                param_files = glob.glob("全量工参/ProjectParameter_mongoose*.xlsx")
                if param_files:
                    from datetime import datetime
                    ...
                    if latest_file:
                        ...
                        messagebox.showwarning("NR小区查询", "未找到全量工参文件！")
                        return
                else:
                    messagebox.showwarning("NR小区查询", "未找到全量工参文件！")
                    return

            # Load NR data from the parameter file
            try:
                import pandas as pd
                nr_data = pd.read_excel(param_file, sheet_name='NR Project Parameters')

                # Identify column names first to avoid repeated lookups
                gnodeb_col = 'gNodeB标识\ngNodeB ID\nLong:[0..1048575]'
                cellname_col = '小区名称\nCELL NAME\nString[0..128]'

                # Try to find columns using fuzzy matching if exact names don't exist
                if gnodeb_col not in nr_data.columns:
                    gnodeb_col = self._find_column_by_fuzzy_match(nr_data, 'gNodeB标识') or gnodeb_col
                if cellname_col not in nr_data.columns:
                    cellname_col = self._find_column_by_fuzzy_match(nr_data, '小区名称') or cellname_col

                # Cache string conversions to avoid repeated operations
                gnodeb_values = nr_data[gnodeb_col].astype(str)
                cellname_values = nr_data[cellname_col].astype(str)

                # Filter data - support both gNodeB_CellID and 小区名称 in one search
                if '_' in search_value:
                    # Check if it's in gNodeBID_CellID format (both parts are numeric)
                    parts = search_value.split('_', 1)
                    if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
                        # It's a gNodeBID_CellID format
                        gnodeb_id, cell_id = parts
                        # Filter by both gNodeB ID and Cell ID
                        results = nr_data[
                            (gnodeb_values == gnodeb_id) &
                            (nr_data['小区标识\ncellLocalId\nInteger:[0~2147483647]'].astype(str) == cell_id)
                        ]
                    else:
                        # It's a cell name containing underscores, search as cell name
                        # Search with both exact match (for full names) and partial match (for parts of names)
                        exact_cellname_match = (cellname_values == search_value)
                        partial_cellname_match = cellname_values.str.contains(search_value, na=False, regex=False)

                        results = nr_data[exact_cellname_match | partial_cellname_match]
                else:
                    # For single value, search in both gNodeB ID and 小区名称 with both exact and partial matching
                    # Search with both exact match (for full names and IDs) and partial match (for parts of names)
                    # First, try exact match for gNodeB ID and 小区名称
                    exact_gnodeb_match = (gnodeb_values == search_value)
                    exact_cellname_match = (cellname_values == search_value)

                    # Then, try partial match for 小区名称 only (since partial gNodeB ID might return too many results)
                    partial_cellname_match = cellname_values.str.contains(search_value, na=False, regex=False)

                    results = nr_data[exact_gnodeb_match | exact_cellname_match | partial_cellname_match]

                if results.empty:
                    messagebox.showinfo("NR小区查询", f"未找到匹配的NR小区 (查询值: {search_value})")
                else:
                    # Add results to the treeview
                    for idx, row in results.iterrows():
                        # Use fuzzy matching to find columns by keyword
                        gnodeb_id_col = self._find_column_by_fuzzy_match(results, 'gNodeB标识') or 'N/A'
                        gnodeb_id = str(row.get(gnodeb_id_col, 'N/A'))

                        cell_lon_col = self._find_column_by_fuzzy_match(results, '小区经度') or 'N/A'
                        cell_lon = str(row.get(cell_lon_col, 'N/A'))

                        cell_lat_col = self._find_column_by_fuzzy_match(results, '小区纬度') or 'N/A'
                        cell_lat = str(row.get(cell_lat_col, 'N/A'))

                        cell_name_col = self._find_column_by_fuzzy_match(results, '小区名称') or 'N/A'
                        cell_name = str(row.get(cell_name_col, 'N/A'))

                        cell_id_col = self._find_column_by_fuzzy_match(results, '小区标识') or 'N/A'
                        cell_id = str(row.get(cell_id_col, 'N/A'))

                        antenna_height_col = self._find_column_by_fuzzy_match(results, '天线挂高') or 'N/A'
                        antenna_height = str(row.get(antenna_height_col, 'N/A'))

                        antenna_azimuth_col = self._find_column_by_fuzzy_match(results, '天线方向角') or 'N/A'
                        antenna_azimuth = str(row.get(antenna_azimuth_col, 'N/A'))

                        coverage_type_col = self._find_column_by_fuzzy_match(results, '小区覆盖类型') or 'N/A'
                        coverage_type = str(row.get(coverage_type_col, 'N/A'))

                        ssb_freq_col = self._find_column_by_fuzzy_match(results, 'SSB频点') or 'N/A'
                        ssb_freq = str(row.get(ssb_freq_col, 'N/A'))

                        pci_col = self._find_column_by_fuzzy_match(results, '物理小区识别码') or 'N/A'
                        pci = str(row.get(pci_col, 'N/A'))

                        mnc_col = self._find_column_by_fuzzy_match(results, '移动网络码') or 'N/A'
                        mnc = str(row.get(mnc_col, 'N/A'))

                        group_1_col = self._find_column_by_fuzzy_match(results, '第一分组') or 'N/A'
                        group_1 = str(row.get(group_1_col, 'N/A'))

                        subnet_id_col = self._find_column_by_fuzzy_match(results, '子网ID') or 'N/A'
                        subnet_id = str(row.get(subnet_id_col, 'N/A'))

                        # Add the row to the treeview
                        self.nr_result_tree.insert('', 'end', values=(gnodeb_id, cell_lon, cell_lat, cell_name, cell_id,
                                                                       antenna_height, antenna_azimuth, coverage_type,
                                                                       ssb_freq, pci, mnc, group_1, subnet_id))

            except Exception as e:
                messagebox.showerror("NR小区查询", f"查询NR小区时出错: {str(e)}")

        except Exception as e:
            messagebox.showerror("NR小区查询", f"NR小区查询过程中发生错误: {str(e)}")

    def search_lte_cells(self):
        """Search for LTE cells based on selected field and value"""
        try:
            # Clear previous results
            for item in self.lte_result_tree.get_children():
                self.lte_result_tree.delete(item)

            # Get the search parameters
            search_value = self.lte_search_value.get()

            if not search_value:
                messagebox.showwarning("LTE小区查询", "请输入查询值！")
                return
            ...
            if not param_file:
                ...
                if param_files:
                    ...
                    if latest_file:
                        ...
                        messagebox.showwarning("LTE小区查询", "未找到全量工参文件！")
                        return
                else:
                    messagebox.showwarning("LTE小区查询", "未找到全量工参文件！")
                    return

            # Load LTE data from the parameter file
            try:
                import pandas as pd
                lte_data = pd.read_excel(param_file, sheet_name='LTE Project Parameters')
                ...
                # Try to find columns using fuzzy matching if exact names don't exist
                if enodeb_col not in lte_data.columns:
                    enodeb_col = self._find_column_by_fuzzy_match(lte_data, 'eNodeB标识') or enodeb_col
                if cellname_col not in lte_data.columns:
                    cellname_col = self._find_column_by_fuzzy_match(lte_data, '小区名称') or cellname_col

                # Cache string conversions to avoid repeated operations
                enodeb_values = lte_data[enodeb_col].astype(str)
                cellname_values = lte_data[cellname_col].astype(str)

                # Filter data - support both eNodeB_CellID and 小区名称 in one search
                if '_' in search_value:
                    # Check if it's in eNodeBID_CellID format (both parts are numeric)
                    parts = search_value.split('_', 1)
                    if len(parts) == 2 and parts[0].isdigit() and parts[1].isdigit():
                        # It's a eNodeBID_CellID format
                        enodeb_id, cell_id = parts
                        # Filter by both eNodeB ID and Cell ID
                        results = lte_data[
                            (enodeb_values == enodeb_id) &
                            (lte_data['小区标识\ncellLocalId\ninteger:[0~2147483647]'].astype(str) == cell_id)
                        ]
                    else:
                        # It's a cell name containing underscores, search as cell name
                        ...
                        results = lte_data[exact_cellname_match | partial_cellname_match]
                else:
                    # For single value, search in both eNodeB ID and 小区名称 with both exact and partial matching
                    # Search with both exact match (for full names and IDs) and partial match (for parts of names)
                    # First, try exact match for eNodeB ID and 小区名称
                    exact_enodeb_match = (enodeb_values == search_value)
                    exact_cellname_match = (cellname_values == search_value)

                    # Then, try partial match for 小区名称 only (since partial eNodeB ID might return too many results)
                    partial_cellname_match = cellname_values.str.contains(search_value, na=False, regex=False)

                    results = lte_data[exact_enodeb_match | exact_cellname_match | partial_cellname_match]

                if results.empty:
                    messagebox.showinfo("LTE小区查询", f"未找到匹配的LTE小区 (查询值: {search_value})")
                else:
                    # Add results to the treeview
                    for idx, row in results.iterrows():
                        # Use fuzzy matching to find columns by keyword
                        enodeb_id_col = self._find_column_by_fuzzy_match(results, 'eNodeB标识') or 'N/A'
                        enodeb_id = str(row.get(enodeb_id_col, 'N/A'))
                        ...
                        earfcn_col = self._find_column_by_fuzzy_match(results, '中心载频') or 'N/A'  # Using more general term to match '下行链路的中心载频'
                        earfcn = str(row.get(earfcn_col, 'N/A'))

                        pci_col = self._find_column_by_fuzzy_match(results, '物理小区识别码') or 'N/A'
                        pci = str(row.get(pci_col, 'N/A'))

                        tac_col = self._find_column_by_fuzzy_match(results, '跟踪区码') or 'N/A'
                        tac = str(row.get(tac_col, 'N/A'))
                        ...
                        # Add the row to the treeview
                        self.lte_result_tree.insert('', 'end', values=(enodeb_id, cell_lon, cell_lat, cell_name, cell_id,
                                                                       antenna_height, antenna_azimuth, coverage_type,
                                                                       earfcn, pci, tac, group_1, subnet_id))

            except Exception as e:
                messagebox.showerror("LTE小区查询", f"查询LTE小区时出错: {str(e)}")

        except Exception as e:
            messagebox.showerror("LTE小区查询", f"LTE小区查询过程中发生错误: {str(e)}")

    def copy_nr_cell_value(self):
        """Copy the selected cell value from NR result tree to clipboard"""
        try:
            # Get selected item
            selection = self.nr_result_tree.selection()
            if not selection:
                return
            
            # Get the clicked column (we need to track this differently in Tkinter)
            item = selection[0]
            values = self.nr_result_tree.item(item, 'values')
            if values and self.nr_clicked_column is not None:
                # Copy to clipboard using the treeview's window
                root = self.nr_result_tree.winfo_toplevel()
                root.clipboard_clear()
                # Use the stored column index, default to 0 if out of range
                col_index = self.nr_clicked_column if self.nr_clicked_column < len(values) else 0
                copied_value = str(values[col_index])
                root.clipboard_append(copied_value)
                
                # Show tooltip notification instead of messagebox
                x = root.winfo_pointerx()
                y = root.winfo_pointery() - 30  # Show above cursor
                self.tooltip.show(f"已复制: {copied_value}", x, y, duration=2000)
        except Exception as e:
            messagebox.showerror("复制失败", f"复制时出错: {str(e)}")

    def copy_lte_cell_value(self):
        """Copy the selected cell value from LTE result tree to clipboard"""
        try:
            # Get selected item
            selection = self.lte_result_tree.selection()
            if not selection:
                return
            
            # Get the clicked column (we need to track this differently in Tkinter)
            item = selection[0]
            values = self.lte_result_tree.item(item, 'values')
            if values and self.lte_clicked_column is not None:
                # Copy to clipboard using the treeview's window
                root = self.lte_result_tree.winfo_toplevel()
                root.clipboard_clear()
                # Use the stored column index, default to 0 if out of range
                col_index = self.lte_clicked_column if self.lte_clicked_column < len(values) else 0
                copied_value = str(values[col_index])
                root.clipboard_append(copied_value)
