"""
邻区规划服务 - 基于NetworkPlanningTool_V1.py的逻辑重构
"""
import math
...
class NeighborPlanningService:
    ...
    def calculate_bearing(self, lon1: float, lat1: float, lon2: float, lat2: float) -> float:
        """计算从点1到点2的方位角（度）"""
        return DistanceCalculator.calculate_bearing(Point(lon1, lat1), Point(lon2, lat2))
    ...
    def calculate_neighbor_score(self, source: SectorInfo, target: SectorInfo,
                                 distance: float, bearing: float) -> float:
        """计算邻区得分（用于排序）"""
        score = 0.0
        ...
        if self.config.source_type == self.config.target_type:
            if source.earfcn and target.earfcn and abs(source.earfcn - target.earfcn) < 0.1:
                score += 20

        return score

    def is_in_direction(self, source: SectorInfo, target: SectorInfo, bearing: float) -> bool:
        """检查目标小区是否在源小区的扇区方向内"""
        angle_diff = abs(bearing - source.azimuth)
        if angle_diff > 180:
            angle_diff = 360 - angle_diff
    ...
    def find_neighbors_for_sector(self, source: SectorInfo, target_sectors: List[SectorInfo]) -> List[NeighborRelation]:
        """为指定小区查找邻区"""
        candidates = []
        ...
        for target in target_sectors:
            ...
            # 方向过滤
            if not self.is_in_direction(source, target, bearing):
                continue

            # 计算得分
            score = self.calculate_neighbor_score(source, target, distance, bearing)
    ...
    def add_co_site_neighbors(self, neighbors: List[NeighborRelation], source: SectorInfo,
                              target_sectors: List[SectorInfo]) -> List[NeighborRelation]:
        """添加同站邻区"""
    ...
    async def plan(
        ...
        Returns:
            规划结果
        """
        task_id = f"neighbor_{int(asyncio.get_event_loop().time())}"
        ...
        for site in sites_data:
            network_type_str = site.get('networkType', 'LTE')
            network_type = NetworkType.LTE if network_type_str == 'LTE' else NetworkType.NR
            ...
            for sector_data in site.get('sectors', []):
                ...
                # 根据类型分类
                if network_type == self.config.source_type:
                    source_sectors.append(sector)

                if network_type == self.config.target_type:
        ...
        for site_id, sectors in site_sectors.items():
            ...
            for sector in sectors:
                # 查找邻区
                neighbors = self.find_neighbors_for_sector(sector, target_sectors)

                # 添加同站邻区
                neighbors = self.add_co_site_neighbors(neighbors, sector, target_sectors)
                ...
                if len(neighbors) < self.config.min_neighbors:
                    ...
                    for target in target_sectors:
                        ...
                        if distance <= self.config.max_distance:
                            all_candidates.append((target, distance))

                    all_candidates.sort(key=lambda x: x[1])

                    for target, dist in all_candidates:
                        if len(neighbors) >= self.config.min_neighbors:
                            break
                        if target.id not in [n.target_sector for n in neighbors]:
                            bearing = self.calculate_bearing(
                                sector.longitude, sector.latitude,
                                target.longitude, target.latitude
                            )
                            neighbors.append(NeighborRelation(
                                source_sector=sector.id,
